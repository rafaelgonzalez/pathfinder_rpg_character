RSpec.describe PathfinderRpg::Character::Klass::Sorcerer do
  ...
  describe '.label' do
    it { expect(described_class.label).to eql 'sorcerer' }
  end

  describe '#label' do
    it { expect(subject.label).to eql 'sorcerer' }
  end
  ...
  describe '.hit_die' do
    it { expect(described_class.hit_die).to eql 6 }
  end

  describe '#hit_die' do
    it { expect(subject.hit_die).to eql 6 }
  end
  ...
  describe '#base_attack_bonus' do
    ...
    context 'at level 5' do
      ...
      it { expect(subject.base_attack_bonus).to eql 2 }
      it { expect(subject.base_attack_bonus(1)).to eql 2 }
  ...
  describe '#fortitude_save_base' do
    context 'at level 1' do
      ...
      it { expect(subject.fortitude_save_base).to eql 0 }
    end

    context 'at level 5' do
      let(:level) { 5 }

      it { expect(subject.fortitude_save_base).to eql 1 }
    end

    context 'at level 10' do
      let(:level) { 10 }

      it { expect(subject.fortitude_save_base).to eql 3 }
    end

    context 'at level 15' do
      let(:level) { 15 }

      it { expect(subject.fortitude_save_base).to eql 5 }
    end

    context 'at level 20' do
      let(:level) { 20 }

      it { expect(subject.fortitude_save_base).to eql 6 }
    end
  end
